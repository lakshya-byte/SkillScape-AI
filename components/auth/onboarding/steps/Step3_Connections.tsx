...
export default function Step3_Connections() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(".platform-card",
        { y: 40, opacity: 0 },
        { y: 0, opacity: 1, duration: 0.6, stagger: 0.1, delay: 0.2, ease: "back.out(1.2)" }
      );
    }, containerRef);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <div ref={containerRef} className="w-full max-w-5xl mx-auto flex flex-col items-center relative z-10">
      ...
      <div className="w-full max-w-4xl mb-12">
        ...
        <div className="flex justify-between items-end mb-4 header-el">
           ...
           <div className="flex items-center gap-4">
             <span className="text-[10px] font-mono text-slate-500 uppercase tracking-widest bg-white/5 px-2 py-1 rounded">Step 4 of 6</span>
             <span className="text-sm font-bold text-purple-400">60%</span>
           </div>
        </div>

        {/* Title & Progress */}
        <h2 className="header-el text-4xl md:text-5xl font-bold text-white mb-6 tracking-tight">Connect Your Ecosystem</h2>
        <p className="header-el text-slate-400 max-w-xl text-sm md:text-base mb-8 leading-relaxed">
           VelionAI requires access to your technical footprint to generate intelligence. 
           Plug in your modules below to synchronize your data streams.
        </p>
          ...
          return (
            ...
            >
              ...
              <div className="relative h-full bg-[#0E0E14] rounded-[22px] p-6 flex flex-col items-center justify-between min-h-[280px] overflow-hidden">
                ...
                <div className={`absolute inset-0 border-2 rounded-[22px] transition-colors duration-500 pointer-events-none ${isConnected ? 'border-emerald-500/50' : 'border-transparent group-hover:border-white/10'}`} />

                {/* Status Dot */}
                <div className="w-full flex justify-end">
                   <div className={`w-2 h-2 rounded-full transition-colors duration-300 ${isConnected ? 'bg-emerald-500 shadow-[0_0_8px_#10B981]' : 'bg-slate-800'}`} />
                </div>

                {/* Icon & Text */}
                <div className="flex flex-col items-center text-center gap-4">
                  <div className={`w-16 h-16 rounded-2xl flex items-center justify-center bg-white/5 border border-white/5 transition-colors duration-300 ${isConnected ? 'bg-emerald-500/10 border-emerald-500/20' : ''}`}>
                    <platform.icon 
                      size={32} 
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-mono uppercase tracking-wider">{platform.desc}</p>
                  </div>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex flex-col items-center text-center mb-8">
                {/* Platform Icon */}
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-4 ${selectedPlatform.bg} ${selectedPlatform.color} border border-white/5`}>
                   <selectedPlatform.icon size={32} />
                </div>
                
                <h3 className="text-2xl font-bold text-white mb-2">Connect {selectedPlatform.name}</h3>
