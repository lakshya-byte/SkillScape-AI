...
import { User, Mail, Building2, ArrowRight, IdCard } from 'lucide-react';
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(".form-element",
        ...
        { y: 0, opacity: 1, filter: "blur(0px)", duration: 0.4, stagger: 0.08, delay: 0.2, ease: "back.out(1.2)" }
      );
    }, containerRef);
    ...
    <div ref={containerRef} className="w-full max-w-[480px] mx-auto opacity-0">
      ...
      <div className="relative bg-[#0A0A0F]/80 backdrop-blur-2xl border border-white/5 rounded-[32px] p-8 md:p-10 shadow-2xl overflow-hidden group">
        ...
        <div className="form-element flex justify-between items-center mb-3">
            <span className="text-[10px] font-mono font-bold text-slate-500 tracking-widest uppercase">Step 2 of 6</span>
            <span className="text-[10px] font-mono font-bold text-purple-400">20%</span>
        </div>
        ...
        <div className="form-element flex flex-col items-center text-center mb-10">
            ...
            <h2 className="text-3xl font-bold text-white mb-2 tracking-tight">Basic Identity</h2>
            <p className="text-sm text-slate-400 max-w-[260px] mx-auto leading-relaxed">
                To begin utilizing VelionAI technical intelligence, please verify your credentials.
            </p>
        </div>
        ...
        <div className="space-y-6">
            ...
            <div className="form-element space-y-2">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider pl-1 flex items-center gap-2">
                    Full Name
                    {isNameValid && <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-emerald-500 text-[10px]">✓</motion.span>}
                </label>
                <div className="relative group">
                    <User className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors duration-300 ${activeField === 'name' ? 'text-purple-400' : 'text-slate-600'}`} />
                    <input 
                        type="text" 
                        value={userData.name}
                    ...
                    <div className={`absolute inset-0 rounded-xl border pointer-events-none transition-colors duration-300 ${activeField === 'name' ? 'border-purple-500/50' : 'border-transparent'}`} />
                    <motion.div 
            ...
            <div className="form-element space-y-2">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider pl-1 flex items-center gap-2">
                    Primary Email
                    {isEmailValid && <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-emerald-500 text-[10px]">✓</motion.span>}
                </label>
                <div className="relative group">
                    <Mail className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors duration-300 ${activeField === 'email' ? 'text-purple-400' : 'text-slate-600'}`} />
                    <input 
                        type="email" 
                        ...
                        className="w-full bg-[#050508]/40 border border-white/5 rounded-xl py-4 pl-12 pr-4 text-sm text-white placeholder-slate-700 focus:outline-none focus:bg-[#050508]/80 transition-all duration-300"
                    />
                    <div className={`absolute inset-0 rounded-xl border pointer-events-none transition-colors duration-300 ${activeField === 'email' ? 'border-purple-500/50' : 'border-transparent'}`} />
                    <motion.div 
                        className="absolute bottom-0 left-1/2 h-[1px] bg-purple-500 shadow-[0_0_8px_#A855F7]"
                        initial={{ width: 0, x: "-50%" }}
            ...
            {/* Field: Institution */}
            <div className="form-element space-y-2">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider pl-1">Institution</label>
                <div className="relative group">
                    <Building2 className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors duration-300 ${activeField === 'org' ? 'text-purple-400' : 'text-slate-600'}`} />
                    <input 
                        type="text" 
                        value={userData.org}
                        onChange={(e) => updateUserData({ org: e.target.value })}
                        onFocus={() => setActiveField('org')}
                        onBlur={() => setActiveField(null)}
                        placeholder="Organization or University"
                        className="w-full bg-[#050508]/40 border border-white/5 rounded-xl py-4 pl-12 pr-4 text-sm text-white placeholder-slate-700 focus:outline-none focus:bg-[#050508]/80 transition-all duration-300"
                    />
                    <div className={`absolute inset-0 rounded-xl border pointer-events-none transition-colors duration-300 ${activeField === 'org' ? 'border-purple-500/50' : 'border-transparent'}`} />
                    <motion.div 
                        className="absolute bottom-0 left-1/2 h-[1px] bg-purple-500 shadow-[0_0_8px_#A855F7]"
                        initial={{ width: 0, x: "-50%" }}
                        animate={{ width: activeField === 'org' ? '100%' : '0%' }}
                        transition={{ duration: 0.4, ease: "easeOut" }}
                    />
                </div>
            </div>
        ...
        <div className="form-element mt-10 space-y-5">
            ...
            >
                ...
                <div className="relative flex items-center justify-center gap-2">
                    <span>Continue</span>
                    <ArrowRight className={`w-4 h-4 transition-all duration-300 ${isValid ? 'group-hover:translate-x-1' : ''}`} />
                </div>
            </motion.button>
